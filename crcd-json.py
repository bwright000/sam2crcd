...
TEST_JSON_PATH  = os.path.join(CASE_DIR, "test.json")  # set to None if you don't have it
...
def image_annotation_stats(name, images, annotations):
    ...
    masks_per_image = [len(anns_by_image[img["id"]]) for img in images if img["id"] in anns_by_image]
...
def main():
    ...
    # Assume categories are identical; if not, you’ll see it in the summaries.
    print("\n=== BASIC COUNTS ===")
    ...
    for img in test_images:
        if img["id"] in combined_images_by_id:
            # could optionally check for file_name consistency here
            continue
        combined_images_by_id[img["id"]] = img
    ...
    # ---- Frames on disk (C_1) ----
    num_frames, split_frame_counts = count_frames_on_disk(SPLIT_ROOT)
